...
| Choice resolution | Loaded children are copied into the choice when type-compatible. Update synchronizes the chosen definition into the child occurrence. Defaults can fill blanks; changing a selection erases its derived children. | Mostly documented. | Confirmed. | High | Selection changes must invalidate the selected occurrence's subtree and rerun topology, not patch only the leaf ID. |
...
| Duplicate handling | `CheckPrevious` clears a chosen definition when an equivalent/pass-through definition is already active at the relevant level, except for the rule default and the current replacement source. `Selected` is derived from active membership. | Described as generic duplicate-choice invalidity. | Sharpened. | High | Duplicate policy is contextual, not a blanket unique-ID rule; defaults, pass-through records, level masks, and replacement sources need fixtures. |
