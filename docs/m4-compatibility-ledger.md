...
| Companions and deities             | Implemented with recorded risk            | Companion ownership/stat rules and ordinary companion prerequisites are evaluated; deity names/domains and Channel Divinity-style alternatives are structured predicates. Beast-ability power prose no longer silently substitutes the character's ability and reports `companion-ability:<power-id>`. A private native audit can select a beast power but does not materialize its requested companion, so actual companion-score substitution remains an M5 blocker.                                                                                                                       |
...
| Hand-specific and named power math | Partially closed; blocking tail remains   | M5 projects saved hand pairs, implements `Dual Implement Spellcaster`, and projects quarry/curse/sneak attack conditionally. The fixed-output `SpecialCase` slice covers Bond of Censure's tier dice/ability suppression, fixed ongoing-10 forms, and the Dexterity-modifier ongoing branch. Same-profile public differentials cover each branch; Hu's supplied exact-profile cache is 52/52 after Bond closure, and a level-5 private native selection audit covers the Dexterity branch. Unknown striker variants and five identified native special entities stay explicitly unsupported. |
