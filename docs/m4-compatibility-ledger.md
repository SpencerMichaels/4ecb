...
| Hand-specific and named power math | Partially closed; blocking tail remains   | M5 projects saved hand pairs, implements `Dual Implement Spellcaster`, and projects quarry/curse/sneak attack conditionally. The fixed-output `SpecialCase` slice now covers Bond of Censure's tier dice/ability suppression plus the recovered fixed ongoing-10 hit forms. Same-profile public differentials cover each branch; Hu's supplied exact-profile cache is 52/52 after Bond closure. Unknown striker variants and the remaining native special strings stay explicitly unsupported. |
