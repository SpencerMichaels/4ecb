...
| Hand-specific and named power math | Partially closed; blocking tail remains   | M5 now projects the saved main-hand choice into explicit main/off-hand pairs and implements the named `Dual Implement Spellcaster` damage bonus from the distinct equipped off-hand implement. Same-profile public differentials swap unequal enhancements and require the selected hands and provenance to change exactly. The remaining native `SpecialCase` tail and situational striker/conditional components still need same-profile differential fixtures. Preserved power text remains visible; the engine never fabricates certainty. |
